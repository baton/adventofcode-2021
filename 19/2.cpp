...
int main( int argc, char * argv[] )
{
	...
	std::vector< scanner_t > mscanners;
	...
	uint64_t max_d( 0 );
	for( size_t i( 0 ); i != mscanners.size() - 1; ++i )
		for( size_t j( i + 1 ); j != mscanners.size(); ++j )
		{
			point_t const dp( mscanners[ i ].position - mscanners[ j ].position );
			uint64_t const dd( std::abs( dp.x ) + std::abs( dp.y ) + std::abs( dp.z ) );
			if( max_d < dd )
				max_d = dd;
		}

	std::cout << "result = " << max_d << std::endl;
